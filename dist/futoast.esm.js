...
var futoast = createCommonjsModule(function (module, exports) {
  (function (global, factory) {
    ...
    class Futoast {
      ...
      createToast(message, options) {
        ...
        const closeButton = options.closeButton ? `<button class="futoast-close" aria-label="Close" tabindex="0">&times;</button>` : '';
        let buttonsHtml = '';
      ...
      show(message, options = {}) {
        return new Promise(resolve => {
          ...
          const adjustedPosition = this.getAdjustedPosition(mergedOptions.position, mergedOptions.mobilePosition);
          this.setContainerPosition(adjustedPosition, mergedOptions.zIndex);
          if (mergedOptions.overlay) {
            this.createOverlay(mergedOptions.overlayColor, mergedOptions.zIndex, mergedOptions.overlayClose);
          }
          ...
          if (mergedOptions.timeout > 0) {
            this.setupProgressBar(toastEl, mergedOptions.timeout, () => {
          ...
          if (mergedOptions.onClick) {
            toastEl.addEventListener('click', e => {
              if (e.target !== toastEl.querySelector('.futoast-close') && !e.target.closest('.futoast-buttons')) {
    ...
    const methods = ['show', 'primary', 'secondary', 'success', 'error', 'warning', 'info', 'question', 'loading', 'clear', 'applyTheme', 'setIcons', 'setThemeMode', 'setFont'];
    ...
    if (typeof angular !== 'undefined') {
      angular.module('futoast', []).factory('Futoast', function () {
        return instance;
      });
      instance.Angular = {
        moduleName: 'futoast'
